...
def train_model(data_path, target_column, model_name):
    ...
    """
    data = pd.read_csv(data_path)  # Load data from file
...
def train():
    ...
    columns = list(pd.read_csv(data_path).columns) # Get columns from file
    ...
    if form.validate_on_submit():
        ...
        model_path, score = train_model(data_path, target_column, model_name) # Get model path
        ...
        if model_path:
            ...
            session['model_path'] = model_path # Store ONLY the path
            session['score'] = score
